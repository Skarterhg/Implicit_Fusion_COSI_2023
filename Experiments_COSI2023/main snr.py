...
parser.add_argument("--snr", type=int, default=-1, help="SNR of the experiment")
...
if debug:
    ...
    results_path =  os.path.join("new_range_results_pavia_lowerpe", "snr_results","optim_parameters_snr_1_5")
...
callbacks = LogTrainingCallback(full_dataset=test_loader, exp_folder = results_path+ f"noisestd={snr}",eval_loss=evaluation ,shape_data = imshapegt, log_scalars_freq = 1, log_images_freq = 1, write_images = True, write_scalars = True, device = device,start=start,end=end,number=numberofbands)
